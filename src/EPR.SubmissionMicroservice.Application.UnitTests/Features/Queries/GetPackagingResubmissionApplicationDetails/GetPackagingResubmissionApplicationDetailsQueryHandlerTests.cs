...
public class GetPackagingResubmissionApplicationDetailsQueryHandlerTests
{
    ...
    [TestMethod]
    public async Task Handle_ShouldNotReportDeclaration_WhenRegulatorHasRejectedTheDeclaredCycle()
    {
        // Arrange - user declared cycle 1, regulator rejected it. Without the regulator-decision cycle-closer
        // the declaration would still be reported on this visit, and FileUploadSubLandingController would
        // bounce the user to ResubmissionTaskList instead of the UploadNewFileToSubmit intermediate page.
        var submissionId = Guid.NewGuid();
        var fileA = Guid.NewGuid();
        var now = DateTime.Now;
    ...
    [TestMethod]
    public async Task Handle_ShouldNotReportDeclaration_WhenRegulatorHasAcceptedTheDeclaredCycle()
    {
        // Arrange - user declared cycle 1, regulator accepted it. On the next visit (user wants to start a
        // fresh cycle) the previous cycle's declaration must not be reported, or the frontend will bounce
        // the user to ResubmissionTaskList and hide the UploadNewFileToSubmit intermediate page.
        var submissionId = Guid.NewGuid();
        var fileA = Guid.NewGuid();
        var now = DateTime.Now;
